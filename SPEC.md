...
**One-line positioning:** Draw titles from your stack and mark each one finished when the watch ends.

| Field | Value |
| --- | --- |
| Product name | Splicer |
| Bundle identifier | `com.splicer.lane` |
| Domain | https://splicer-lane.pro |
| Contact URL | https://splicer-lane.pro/contact-us |
| Deployment target | iOS 17.0 |
| Swift version | 6.2, strict concurrency `complete` |
| Devices | iPhone and iPad, portrait |
| Interface style | Dark |
| Asset prefix | `spl_` |
| User-Agent | `Splicer/1.0 (iOS; +https://splicer-lane.pro)` |
...
8. **The app must build clean** with
   `xcodegen generate && xcodebuild -scheme Splicer -destination 'generic/platform=iOS' build`.
...
A viewer draws the top title from the stack lane onto the gate and burns it when the watch finishes so the backlog shrinks instead of growing a seen journal.

### 2.1 User flow

1. Search a title and stash it on the stack lane
2. Draw the top stack stub onto the gate
3. Burn the gate stub when the watch finishes
4. Rate the burn from 1 to 5
5. Open Ash to read burn order, or Profile for taste from burn grades

### 2.2 Essential behaviour

- Two-lane kanban board as the home surface
- Piled / Gated / Empty fold over stubs with Draw and Burn verbs
- BurnMark on burn; rate fuses only on BurnMark
- Ash lists BurnMarks in order; Profile folds burn grades only
- Discover search sheet; local shelf catches empty or failed search; no stream links as a store

---

## 3. Uniqueness assignment for Splicer

| Axis | Assigned value |
| --- | --- |
| Architecture | **Lane ADT fold (Piled | Gated | Empty); the board is a fold over Stubs; Stash writes a Stub on Stack; Draw writes the top Stack Stub onto Gate and folds Piled to Gated; Burn writes a BurnMark when Gate holds the Stub and removes it; Burn from Gated with Stack remaining folds to Piled; Burn from Gated with empty Stack folds to Empty; rate folds only BurnMark; Draw on empty Stack writes Bare; Burn on Piled is refused; Draw on Gated is refused; Stash on Empty folds Empty to Piled; empty board writes Dark** |
| UI approach | **SwiftUI Layout protocol · material** |
| Naming convention | **Celluloid / splice lexicon** |
| File organization | **By lane role (Board, Stack, Gate, Stub, BurnMark, SlipMark, Dark)** |
| Dependency strategy | **None NotificationCenter** |
| Design direction | **dashboard · marquee-band · mid** |
| Typography | **Bodoni 72** |
| Navigation pattern | **Board-locked chrome (the two-lane board never leaves; Discover, Ash and Profile arrive as sheets; stash and draw fuse on Stack)** |
| AI art style | **Geometric pattern tile · collage** |
| Functional twist | **Draw-then-burn (Stash writes a Stub on Stack; Draw moves the top Stack Stub to Gate; Burn writes a BurnMark and removes the Gate Stub; Ash is BurnMark order not a Seen journal; Draw on empty Stack writes Bare; Burn on Piled is refused; a second Draw while Gate holds a Stub is refused)** |
...
**Core** — A viewer draws the top title from the stack lane onto the gate and burns it when the watch finishes so the backlog shrinks instead of growing a seen journal.

**Audience** — People with a saved watch pile who want to burn through titles one at a time on this device, not curate an endless seen shelf or open a streaming store.
...
**Twist** — Draw-then-burn. Home is the two-lane board. Search writes a Stub on Stack. Draw moves the top Stack Stub to Gate and folds Piled to Gated. Burn writes a BurnMark when Gate holds that Stub, removes it from the board, and opens rate; a miss on Gate writes a SlipMark and keeps the stub. Rate folds only BurnMark. Ash is BurnMark order, not a growing Seen journal. Profile counts BurnMarks and burn grades. Draw on empty Stack writes Bare. Burn on Piled is refused. A second Draw while Gate holds a Stub is refused. Seed already Draws one Stub to Gate so opening Burn is live. Home verb: burn-the-gate, not reel-the-bill and not save-a-queue. Local only.

**Why this is not a repeat** — Bioscope is a two-seat bill with reel-the-bill for pairs tonight. Splicer is a solo kanban burn-through: Stack and Gate lanes, draw-then-burn, Ash as burn marks not a seen journal. Different home surface, ADT, navigation chrome, lexicon, and verb. Not the vanilla search→queue→seen→rate clone.

### 3.0a Craft from the shipped portfolio

Full craft is in KNOWLEDGE.md. Follow it. Do not copy type names or layouts.
- Home: Only if the job changes: tonight-for-two, kanban pile, or dated timeline.
- Invariant: Vanilla search→queue→seen→rate is a 10-app clone. Pick one rare verb and persist it on home.
- Never: Not a streaming store.
...
The board is a Lane fold of Piled, Gated, or Empty over Stubs, and an empty board writes Dark. Stash writes a Stub on Stack and folds Empty to Piled. Draw seats the top Stack Stub on Gate and folds Piled to Gated, while Draw on an empty Stack writes Bare and a second Draw while Gate holds a Stub is refused. Burn writes a BurnMark only when Gate holds that Stub, removes it, and folds to Piled if Stack remains or to Empty if Stack is clear, while Burn on Piled is refused and a miss writes a SlipMark that keeps the stub. Rate folds only onto a BurnMark, Ash lists BurnMarks in order, and Profile counts those marks and their grades, so the unit-tested home verb is burn-the-gate.
...
LaneLayout, a SwiftUI Layout, is the one custom surface and places Stack and Gate edge to edge under a full-width marquee band, then the burn verb. The gate is the hero and the stack is a narrower pile of uneven stubs, on an 8pt grid, with the 24pt radius on lanes and the 10pt radius on chips. Elevation is the single shadow token. The marquee is the one translucent plate, and lane bodies stay filled surfaces. Burn uses a bordered-prominent ButtonStyle with default, pressed, disabled, and loading. Deleting a stub uses a destructive style. Discover, Ash, Profile, and Settings are stock sheets. Controls are native Button, Toggle, and TextField, with the whole chrome inside the label, contentShape, and a 44pt minimum. Motion is a 180ms ease-out cross-fade, and Reduce Motion swaps instantly. One haptic fires on a successful burn. Icon-only controls have VoiceOver labels, and color is never the only signal. Dark is a full-page empty state with generated art, one headline, one line, and a full-width Stash control. The board fills the safe area on iPhone and iPad.

### 3.3 Naming contract

Convention: Celluloid / splice lexicon.

Examples to follow: `BoardFold`, `GateStub`, `BurnMark`, `SlipMark`

### 3.4 Dependency contract

No Swift packages, no CocoaPods, and no vendored code. NotificationCenter is the only extra coordination: a successful Stash, Draw, or Burn posts LaneFoldNotice so Ash and Profile reload from the store while BoardFold stays the owner of the fold. URLSession performs the title search. The product does not capture, so the camera is not a feature surface.

### 3.5 Navigation contract

The two-lane board is the permanent root and never pushes away. There is no tab bar. Discover, Ash, Profile, and Settings arrive as sheets. Stash and Draw fuse on Stack. Burn fuses on Gate. Onboarding is a full-page cover with a full-width Continue at the bottom, then a completion flag. After onboarding, read ProcessInfo arguments once. The launch key today shows Board, log presents Ash, goals presents Profile, discover presents Discover, and settings presents Settings, each a different surface. Simulator seed spl.demo.v1 marks onboarding complete, places four local-shelf stubs on the board, and draws one of them onto the gate so Burn is enabled.

### 3.6 Screen composition contract

Board-root fused kanban (Stack holds draw and burn; Discover, Ash and Profile are sheets; Queue is not a pushed scene)

Physical screens: Board, Discover, Ash, Profile, Settings, Onboarding. Board is the home mechanic, a marquee band over Stack and Gate, filling the device. Discover is a search sheet: a debounced title query to cgi search pl with the Splicer user agent, mapped to Stub titles, and the bundled local shelf when the query is empty or the request fails, with no play links. Ash is a sheet of BurnMarks in burn order and uses a full-page empty state. Profile is a sheet of burn counts and grades only. Settings holds https://splicer-lane.pro/contact-us, resetAllData, and re-run onboarding. Onboarding is three pages and not a destination on the board. Queue is not a pushed scene.
...
Scheme: **By lane role (Board, Stack, Gate, Stub, BurnMark, SlipMark, Dark)**

```
Splicer/
  Board/BoardFold.swift
Board/LaneLayout.swift
Board/SettingsSheet.swift
Board/LaneChartStore.swift
Board/SpliceTokens.swift
Stack/StackLane.swift
Stack/DiscoverSheet.swift
Gate/GateLane.swift
Stub/Stub.swift
BurnMark/BurnMark.swift
BurnMark/AshSheet.swift
BurnMark/ProfileSheet.swift
SlipMark/SlipMark.swift
Dark/DarkEmpty.swift
  Assets.xcassets/
```
...
### 5.3 Queue
A first-class screen for **Queue**. Must render empty, populated and error states.
...
- **QueueItem** — named per this app's convention.
...
Direction: **dashboard · marquee-band · mid**

### 7.1 Palette

| Token | Hex | Use |
| --- | --- | --- |
| `background` | `#09090B` | Screen background, the void |
| `surface` | `#18181B` | Cards, rows, sheets, one step above the void |
| `ink` | `#FAFAFA` | Primary text, icons, figures, focus ring |
| `accent` | `#0C5CAB` | Primary action fill. Not small type on the void |
| `muted` | `#A1A1AA` | Secondary text, dividers, disabled |
...
Bodoni 72 is the only UI face, set through one accessor as Font.custom with six steps: display, title, headline, body, caption, and micro. Bodoni 72 Bold is the marquee and the gate title. Bodoni 72 Book is body and captions. There is no second display face. Figures are tabular: grades, stub counts, and day numbers go through NumberFormatter with monospaced digits. One hairline rule sits under the marquee. No step exceeds 34pt, body stays on the 17pt step, and every step tracks Dynamic Type with ScaledMetric. At the largest accessibility size, long lines fall back to New York so hairline serifs stay readable. Day edges pass through Calendar.current.startOfDay before they become a YYYYMMDD int.
...
This app's `ui` axis is **SwiftUI Layout protocol · material**.
...
If **SwiftUI Layout protocol · material** is already fully native (no custom drawing layer), this section is satisfied automatically — there is nothing to confine.
...
Reference system: **dashboard** — palette, density, and component language through these tokens and the 24/10 radii. Not web chrome, type names, logos, or product logic.

Mood: **Dark-themed cloud-platform aesthetic with modular grids, glass-like panels, and strong data hierarchy for productivity dashboards.**.

Home rhythm (`marquee-band`, airy): A full-width colour or media band resets the rhythm, then the verb.

Dark-tech: void surfaces, one glow or jewel, sparse chrome. Layout `marquee-band`, density airy. Kit 24/10, shadow, bordered prominent. Palette recipe `mid`. Almost no travel. Cross-fade 180ms ease-out. Numbers tick, they do not fly. Reduce Motion: instant swap. Reduce Motion: fade only. Do not invent a second radius or a second accent.
...
UserDefaults holds one Codable Chart root, LaneChart, under the single key spl.lane.v1. LaneChart holds the Stack of Stubs, the Gate stub when one is seated, BurnMarks, SlipMarks, and the grades that fold only onto BurnMarks. Each mark triggers a debounced save of that one record. A BurnMark stores its day as an Int in YYYYMMDD form, derived with Calendar.current.startOfDay. The screen layer reads and writes through one store seam, and resetAllData deletes the key from Settings.
...
Set `User-Agent: Splicer/1.0 (iOS; +https://splicer-lane.pro)` on every request. Never reuse another app's string.
...
## 12. Functional twist: Draw-then-burn (Stash writes a Stub on Stack; Draw moves the top Stack Stub to Gate; Burn writes a BurnMark and removes the Gate Stub; Ash is BurnMark order not a Seen journal; Draw on empty Stack writes Bare; Burn on Piled is refused; a second Draw while Gate holds a Stub is refused)

Home is the two-lane board, and the job is to burn the title on the gate when the watch finishes. Search writes a Stub on Stack, Draw moves only that top stub onto the gate, and Burn writes a BurnMark, removes the stub, and opens a grade from 1 to 5. Ash stores those BurnMarks in order so the backlog shrinks. Draw on an empty Stack writes Bare, Burn on Piled is refused, and a second Draw while the gate is occupied is refused. The Simulator seed behind spl.demo.v1 already draws one of four local-shelf stubs onto the gate, so Burn is enabled on the first frame and a refused Draw is only a unit-test fixture. Discover stashes titles from search or the local shelf and offers no stream link.
...
Art style: **Geometric pattern tile · collage**


Base prompt, reused and extended for every asset:

```
Geometric pattern tile collage. Hard-edged tiles repeat in a tight mosaic, then a few tiles are cut and layered like spliced film frames. Subjects are solid and opaque, centered, with clear corners on cutouts. Icon, splash, and backdrop fill the canvas edge to edge. No letters, no hollow frames, no wire outlines.
```
...
given. Every name carries the `spl_` prefix.
...
| --- | --- | --- | --- | --- |
| 1 | `spl_AppIcon` | 1024x1024 | **NO** | App Store icon. NO alpha channel, NO transparency, NO text, NO rounded corners, NO drop shadow outside the canvas. |
| 2 | `spl_Splash` | 1290x2796 | fill | Launch background. The middle third must stay quiet so the wordmark reads on top. |
| 3 | `spl_Onboarding1` | 1024x1536 | **required cutout** | Onboarding page 1 illustration: what the app is for. |
| 4 | `spl_Onboarding2` | 1024x1536 | **required cutout** | Onboarding page 2 illustration: the main verb. |
| 5 | `spl_Onboarding3` | 1024x1536 | **required cutout** | Onboarding page 3 illustration: why they stay. |
| 6 | `spl_EmptyHome` | 1024x1024 | **required cutout** | Empty state: the home screen has nothing yet. Calm and inviting, never sad. |
| 7 | `spl_EmptyList` | 1024x1024 | **required cutout** | Empty state: a secondary list has no rows. |
| 8 | `spl_CardBackdrop` | 1200x800 | fill | Backdrop art for a primary card. Low contrast so text stays readable. |
| 9 | `spl_ControlFace` | 512x512 | **required cutout** | Custom control artwork used for the primary interactive element. |
| 10 | `spl_TwistHero` | 1024x1024 | **required cutout** | Hero art for the 'Draw-then-burn (Stash writes a Stub on Stack; Draw moves the top Stack Stub to Gate; Burn writes a BurnMark and removes the Gate Stub; Ash is BurnMark order not a Seen journal; Draw on empty Stack writes Bare; Burn on Piled is refused; a second Draw while Gate holds a Stub is refused)' feature screen. |
| 11 | `spl_SuccessMark` | 512x512 | **required cutout** | Shown briefly when the primary action succeeds. |
| 12 | `spl_HeaderDecor` | 1200x600 | **required cutout** | Decorative header accent on the main screen. |

### Prompt per asset

**`spl_AppIcon`** — 1024x1024

```
A solid film-gate latch centered on a field of repeating geometric tiles, collage layers overlapping, subject filling the canvas edge to edge, no letters, no rounded mask, opaque square.
```

**`spl_Splash`** — 1290x2796

```
A tall geometric tile collage with a quiet center band, a solid splice block standing in the middle third, tiles reaching all four edges.
```

**`spl_Onboarding1`** — 1024x1536

```
A solid stack of opaque title cards beside a gate frame, geometric tile collage, subject centered, corners clear.
...
**`spl_Onboarding2`** — 1024x1536

```
A solid title card moving from a stack onto a gate, mid gesture, geometric tile collage, opaque forms, corners clear.

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`spl_Onboarding3`** — 1024x1536

```
A short row of solid burned seals in order, geometric tile collage, opaque marks, corners clear.

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`spl_EmptyHome`** — 1024x1024

```
A solid closed film can sitting alone, geometric tile collage, fully opaque metal, not glass, not a hollow frame, corners clear.

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`spl_EmptyList`** — 1024x1024

```
A solid closed folio, geometric tile collage, opaque cover, corners clear.

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`spl_CardBackdrop`** — 1200x800

```
An edge-to-edge geometric tile field, quiet pattern, no competing subject, fills the canvas.
```

**`spl_ControlFace`** — 512x512

```
A solid splice block, the face of the burn control, geometric tile inlay, opaque, centered, corners clear.

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`spl_TwistHero`** — 1024x1024

```
A solid title card halfway from a stack onto a gate, geometric tile collage, opaque, centered, corners clear.

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`spl_SuccessMark`** — 512x512

```
A solid round seal with a thick center, geometric tile inlay, opaque, not a hollow ring, corners clear.

HARD CUTOUT: isolated SOLID opaque subject on a fully transparent background, occupying the center of the canvas. Real PNG alpha channel. All four corners fully transparent. No square plate, no painted backdrop, no opaque box, no drop shadow that fills the canvas. Not glass, not a hollow frame, not a wire outline, not an empty vitrine — rembg punches through those and the cutout is empty. GenerateImage writes opaque RGB — after copy, convert the PNG to RGBA in place; do not generate it again for alpha.
```

**`spl_HeaderDecor`** — 1200x600

```
A wide horizontal cluster of geometric tiles in the center, collage, solid tiles, corners clear.
...
Add a unit test target `SplicerTests` covering at minimum:
...
- [ ] `xcodebuild -scheme Splicer -destination 'generic/platform=iOS' build` succeeds.
...
**Uniqueness**
- [ ] Architecture matches **Lane ADT fold (Piled | Gated | Empty); the board is a fold over Stubs; Stash writes a Stub on Stack; Draw writes the top Stack Stub onto Gate and folds Piled to Gated; Burn writes a BurnMark when Gate holds the Stub and removes it; Burn from Gated with Stack remaining folds to Piled; Burn from Gated with empty Stack folds to Empty; rate folds only BurnMark; Draw on empty Stack writes Bare; Burn on Piled is refused; Draw on Gated is refused; Stash on Empty folds Empty to Piled; empty board writes Dark** with no leakage across layers.
- [ ] UI approach matches **SwiftUI Layout protocol · material**.
- [ ] Custom rendering, if any, is confined to one hero surface (section 7.5).
- [ ] Navigation matches **Board-locked chrome (the two-lane board never leaves; Discover, Ash and Profile arrive as sheets; stash and draw fuse on Stack)**.
...
cd Splicer
xcodegen generate
xcodebuild build-for-testing -scheme Splicer -destination 'generic/platform=iOS Simulator' -jobs 2 CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO -derivedDataPath '/Users/belzephyrus/Documents/gambling-factory/.artifacts/genesis/com.splicer.lane/DerivedData' SWIFT_TREAT_WARNINGS_AS_ERRORS=YES
xcodebuild -scheme Splicer -destination 'generic/platform=iOS' -jobs 2 CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO -derivedDataPath '/Users/belzephyrus/Documents/gambling-factory/.artifacts/genesis/com.splicer.lane/DerivedData' SWIFT_TREAT_WARNINGS_AS_ERRORS=YES build
xcrun simctl list devices available
xcodebuild test-without-building -scheme Splicer -destination 'platform=iOS Simulator,id=<UDID>' -jobs 2 -derivedDataPath '/Users/belzephyrus/Documents/gambling-factory/.artifacts/genesis/com.splicer.lane/DerivedData'
```
